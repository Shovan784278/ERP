...
//            return $pdf->stream('EXAM_SCHEDULE.pdf');
    ...
    public function addExamRoutineStore(Request $request)
    {
        ...
        try {
            ...
            $sections = collect($request->routine)->pluck('section')->unique();

            $exam_schedule  = SmExamSchedule::query();
            if ($class_id) {
                $exam_schedule->where('class_id', $class_id);
            }
           
            $exam_schedule->whereIn('section_id', $sections);
           
            $exam_schedule = $exam_schedule->where('exam_term_id', $exam_term_id)
                ->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->delete();
            ...
            foreach ($request->routine as $routine_data) {
                ...
                $allStudents = StudentRecord::with('student')->where('class_id', $request->class_id)
                            ->where('section_id', gv($routine_data, 'section'))
                            ->get();
                            
                $examType = SmExamType::find($request->exam_type_id);
                foreach($allStudents as $allStudent){
                    $compact['user_email'] = $allStudent->studentDetail->email;
                    $compact['student_name'] = $allStudent->studentDetail->full_name;
                    $compact['parent_name'] = @$allStudent->studentDetail->parents->guardians_name;
                    $compact['exam_type'] = $examType->title;
                    $compact['exam_date'] = date('Y-m-d', strtotime(gv($routine_data, 'date')));
                    $compact['exam_time'] = date('H:i:s', strtotime(gv($routine_data, 'start_time')));
                    $number = @$allStudent->studentDetail->mobile;
                    $parentNumber = @$allStudent->studentDetail->parents->guardians_mobile;
                    @send_sms($number, 'exam_schedule_for_student', $compact);
                    @send_sms($parentNumber, 'exam_schedule_for_parent', $compact);
                }
            }
