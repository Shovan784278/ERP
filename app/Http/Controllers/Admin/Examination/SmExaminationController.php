    ...
    public function sendMarksBySmsStore(Request $request)
    {
        ...
        try {
                ...
                foreach($subjectMarkinfos as $key => $subjectMarkinfo){
                    $students = StudentRecord::with('studentDetail')->find($key);
                    $subjects = '';
                    $marks = '';
                    $subjectMarks = '';
                    ...
                    $compact['student_name'] = $students->studentDetail->full_name;
                    $compact['class_name'] = $students->class->class_name;
                    $compact['section_name'] = $students->section->section_name;
                    $compact['user_email'] = $students->studentDetail->email;
                    $compact['marks'] = $marks;
                    ...
                    if($request->receiver == 'students'){
                        @send_sms($students->studentDetail->mobile, 'exam_mark_student', $compact);
                    }else{
                        $compact['parent_name'] = $students->studentDetail->parents->guardians_name;
                        @send_sms($students->studentDetail->parents->guardians_mobile, 'exam_mark_parent', $compact);
                    }
                }
                Toastr::success('Operation successful', 'Success');
            ...
            if ($request->method() == 'POST') {
                ...
                foreach ($eligible_students as $SingleStudent) {
                    ...
                    if(@$optional_subject_setup){
                        $optional_subject='';
                        $get_optional_subject= SmOptionalSubjectAssign::where('student_id', $SingleStudent->id)
                                                ->where('session_id', $SingleStudent->session_id)
                                                ->pluck('subject_id')
                                                ->toArray();
                        ...
                        ->where('subject_id', '!=', $get_optional_subject)->sum('total_gpa_point');
                    ...
                    if(@$optional_subject_setup){
                        $subject = $results->count()-1;
                        $total_GPA = ($total_gpa_point == 0) ? 0 : $total_gpa_point / $subject;
                    }else{
                    ...
                    if(@$optional_subject_setup){
                        $total_gpa_point_with = SmResultStore::where([
                            ['exam_type_id', $InputExamId],
                            ['class_id', $InputClassId],
                            ['section_id', $InputSectionId],
                            ['student_id', $SingleStudent->id],
                        ...
                        $subject = $results->count()-1;
                        $ttMark = $total_gpa_point_with- $optional_subject_setup->gpa_above;
                        $total_GPA_with = ($total_gpa_point_with == 0) ? 0 :  $ttMark / $subject;
                        $insert_results->result = number_format($total_GPA_with, 2, '.', '');
                    }else{
