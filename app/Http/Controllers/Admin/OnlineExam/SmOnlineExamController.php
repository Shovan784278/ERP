    ...
    public function onlineExamQuestionAssign(Request $request)
    {
        try {
            if ($request->checkbox) {
                ...
                SmOnlineExamQuestionAssign::where([
                    'question_bank_id' => $request->questions,
    ...
    public function onlineExamReportSearch(Request $request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('OnlineExam') == TRUE) {
                ...
                $online_exam = SmOnlineExam::where('class_id', $request->class)->where('section_id', $request->section)->where('id', $request->exam)->first();


                if (!$online_exam) {
                    Toastr::error('Exam not found with your selected options', 'Failed');
                    return redirect('online-exam-report');
                }

                $present_students = [];
                foreach ($students as $student) {
                    $take_exam = SmStudentTakeOnlineExam::where('student_id', $student->id)->where('online_exam_id', $online_exam->id)->first();
                    if ($take_exam != "") {
                        $present_students[] = $student->id;
                    }
                }
                $total_marks = 0;
                foreach ($online_exam->assignQuestions as $assignQuestion) {
                    $total_marks = $total_marks + $assignQuestion->questionBank->marks;
                }
            ...
            return view('backEnd.reports.online_exam_report', compact('online_exam', 'students', 'present_students', 'total_marks', 'exams', 'classes', 'class_id', 'exam_id', 'section_id', 'clas', 'sec'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
